...
function initializeCountdown() {
    ...
    if (!storedLaunchDate) {
        // Initial setup - 3 hours and 30 minutes from now
        const now = new Date();
        launchDate = new Date(now.getTime() + (3.5 * 60 * 60 * 1000)); // 3.5 hours in milliseconds
        // Store it
        localStorage.setItem('solterLaunchDate', launchDate.getTime().toString());
    } else {
